...
final class FineTuneModel: ObservableObject {
    ...
    @Published var model_file_url: URL = .init(filePath: "/")
    @Published var model_file_path: String = "Select model"
    @Published var dataset_file_url: URL = .init(filePath: "/")
    ...
    @Published var lora_file_url: URL = .init(filePath: "/")
    ...
    var llama_finetune: LLaMa_FineTune?
    ...
    func finetune() async {
        Task {
            ...
            let lora_path = String(documents_path!.appendingPathComponent("lora_adapters").path(percentEncoded: true) + lora_name)
//            try! lora_path.write(to: URL(fileURLWithPath: lora_path), atomically: true, encoding: String.Encoding.utf8)
            ...
            tuneQueue.async {
                do {
                    try self.llama_finetune!.finetune { progress_str in
    ...
    func export_lora() async {
        Task {
            ...
            _ = get_path_by_short_name(lora_name, dest: "lora_adapters")
            let lora_path = String(documents_path!.appendingPathComponent("lora_adapters").path(percentEncoded: true) + lora_file_path)
            let export_model_path = String(documents_path!.appendingPathComponent("models").path(percentEncoded: true) + export_model_name)
//            try! lora_path.write(to: URL(fileURLWithPath: lora_path), atomically: true, encoding: String.Encoding.utf8)
            ...
            tuneQueue.async {
                do {
                    try self.llama_finetune!.export_lora { progress in
                        DispatchQueue.main.async {
    ...
    func cancel_finetune() async {
        llama_finetune?.cancel = true
        state = .cancel
    }
}
