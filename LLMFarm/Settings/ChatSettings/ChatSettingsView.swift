...
struct ChatSettingsView: View {
    ...
    @State private var chat_settings_template: ChatSettingsTemplate = .init()
    @State private var chat_setting_templates = get_model_setting_templates()
    @State private var applying_template: Bool = false

    @State private var model_file_url: URL = .init(filePath: "/")
    ...
    @State private var clip_model_file_url: URL = .init(filePath: "/")
    ...
    @State private var lora_adapters: [[String: Any]] = []
    @State private var lora_file_url: URL = .init(filePath: "/")
    ...
    {
        _add_chat_dialog = add_chat_dialog
        _edit_chat_dialog = edit_chat_dialog
        _after_chat_edit = after_chat_edit
         ...
         chat_name: String, after_chat_edit: Binding<() -> Void>, toggleSettings: Binding<Bool>)
    {
        ...
        if chat_config!["title"] != nil {
            _chat_title = State(initialValue: chat_config!["title"]! as! String)
        }
        if chat_config!["model"] != nil {
            _model_file_path = State(initialValue: chat_config!["model"]! as! String)
        }
        if chat_config!["clip_model"] != nil {
            _clip_model_file_path = State(initialValue: chat_config!["clip_model"]! as! String)
            if let _ = get_path_by_short_name(chat_config!["clip_model"]! as! String) {
                _has_clip = State(initialValue: true)
            }
        }
        if chat_config!["lora_adapters"] != nil {
            let adapters = chat_config!["lora_adapters"]! as? [[String: Any]]
            if adapters != nil, adapters!.count > 0 {
                _lora_file_path = State(initialValue: adapters![0]["adapter"]! as! String)
                _lora_file_scale = State(initialValue: adapters![0]["scale"]! as! Float)
                if let _ = get_path_by_short_name(adapters![0]["adapter"]! as! String, dest: "lora_adapters") {
                    _has_lora = State(initialValue: true)
                }
            }
        }
        if chat_config!["icon"] != nil {
            _chat_icon = State(initialValue: chat_config!["icon"]! as! String)
        }
        if chat_config!["model_settings_template"] != nil {
            ...
            for template in chat_setting_templates {
                if template.template_name == cur_template {
                    _chat_settings_template = State(initialValue: template)
                }
            }
        }
        if chat_config!["model_inference"] != nil {
            _model_inference = State(initialValue: chat_config!["model_inference"]! as! String)
        }
        if chat_config!["use_metal"] != nil {
            _use_metal = State(initialValue: chat_config!["use_metal"]! as! Bool)
        }
        if chat_config!["use_clip_metal"] != nil {
            _use_clip_metal = State(initialValue: chat_config!["use_clip_metal"]! as! Bool)
        }
        if chat_config!["mlock"] != nil {
            _mlock = State(initialValue: chat_config!["mlock"]! as! Bool)
        }
        if chat_config!["mmap"] != nil {
            _mmap = State(initialValue: chat_config!["mmap"]! as! Bool)
        }
        if chat_config!["flash_attn"] != nil {
            _flash_attn = State(initialValue: chat_config!["flash_attn"]! as! Bool)
        }
        if chat_config!["prompt_format"] != nil {
            _prompt_format = State(initialValue: chat_config!["prompt_format"]! as! String)
        }
        if chat_config!["warm_prompt"] != nil {
            _warm_prompt = State(initialValue: chat_config!["warm_prompt"]! as! String)
        }
        if chat_config!["reverse_prompt"] != nil {
            _reverse_prompt = State(initialValue: chat_config!["reverse_prompt"]! as! String)
        }
        if chat_config!["skip_tokens"] != nil {
            _skip_tokens = State(initialValue: chat_config!["skip_tokens"]! as! String)
        }
        if chat_config!["numberOfThreads"] != nil {
            _numberOfThreads = State(initialValue: chat_config!["numberOfThreads"]! as! Int32)
        }
        if chat_config!["context"] != nil {
            _model_context = State(initialValue: chat_config!["context"]! as! Int32)
        }
        if chat_config!["n_batch"] != nil {
            _model_n_batch = State(initialValue: chat_config!["n_batch"]! as! Int32)
        }
        if chat_config!["n_predict"] != nil {
            _n_predict = State(initialValue: chat_config!["n_predict"]! as! Int32)
        }
        if chat_config!["top_k"] != nil {
            _model_top_k = State(initialValue: chat_config!["top_k"]! as! Int32)
        }
        if chat_config!["temp"] != nil {
            _model_temp = State(initialValue: chat_config?["temp"] as? Float ?? 0)
            if (chat_config!["temp"] as? Float ?? 0) <= 0 {
                _model_sampling = State(initialValue: "greedy")
            }
        }
        if chat_config!["top_p"] != nil {
            _model_top_p = State(initialValue: chat_config!["top_p"]! as! Float)
        }
        if chat_config!["repeat_penalty"] != nil {
            _model_repeat_penalty = State(initialValue: chat_config!["repeat_penalty"] as? Float ?? 0)
        }
        if chat_config!["repeat_last_n"] != nil {
            _model_repeat_last_n = State(initialValue: chat_config!["repeat_last_n"]! as! Int32)
        }
        if chat_config!["mirostat"] != nil {
            _mirostat = State(initialValue: chat_config!["mirostat"] as! Int32)
            if (chat_config!["mirostat"] as! Int32) == 1 {
                _model_sampling = State(initialValue: "mirostat")
            }
            if (chat_config!["mirostat"] as! Int32) == 2 {
                _model_sampling = State(initialValue: "mirostat_v2")
            }
        }
        if chat_config!["mirostat_tau"] != nil {
            _mirostat_tau = State(initialValue: chat_config!["mirostat_tau"] as! Float)
        }
        if chat_config!["mirostat_eta"] != nil {
            _mirostat_eta = State(initialValue: chat_config!["mirostat_eta"] as! Float)
        }
        if chat_config!["tfs_z"] != nil {
            _tfs_z = State(initialValue: chat_config!["tfs_z"] as! Float)
        }
        if chat_config!["typical_p"] != nil {
            _typical_p = State(initialValue: chat_config!["typical_p"] as! Float)
        }
        if chat_config!["grammar"] != nil {
            _grammar = State(initialValue: chat_config!["grammar"]! as! String)
        }
        if chat_config!["add_bos_token"] != nil {
            _add_bos_token = State(initialValue: chat_config!["add_bos_token"] as! Bool)
        }
        if chat_config!["add_eos_token"] != nil {
            _add_eos_token = State(initialValue: chat_config!["add_eos_token"] as! Bool)
        }
        if chat_config!["parse_special_tokens"] != nil {
            _parse_special_tokens = State(initialValue: chat_config!["parse_special_tokens"] as! Bool)
        }
        if chat_config!["save_load_state"] != nil {
            _save_load_state = State(initialValue: chat_config!["save_load_state"] as! Bool)
        }
        if chat_config!["chat_style"] != nil {
            _chat_style = State(initialValue: chat_config!["chat_style"] as! String)
        }

        // RAG
        _chunkSize = State(initialValue: chat_config?["chunk_size"] as? Int ?? chunkSize)
        _chunkOverlap = State(initialValue: chat_config?["chunk_overlap"] as? Int ?? chunkOverlap)
        _ragTop = State(initialValue: chat_config?["rag_top"] as? Int ?? ragTop)

        if chat_config!["current_model"] != nil {
            _currentModel = State(initialValue: getCurrentModelFromStr(chat_config?["current_model"] as? String ?? ""))
        }
        if chat_config!["comparison_algorithm"] != nil {
            _comparisonAlgorithm = State(initialValue: getComparisonAlgorithmFromStr(chat_config?["comparison_algorithm"] as? String ?? ""))
        }
        if chat_config!["chunk_method"] != nil {
            _chunkMethod = State(initialValue: getChunkMethodFromStr(chat_config?["chunk_method"] as? String ?? ""))
        }
    }
    ...
    func get_chat_options_dict(is_template: Bool = false) -> [String: Any] {
    ...
    func save_chat_settings() {
        ...
        _ = CreateChat(options, edit_chat_dialog: edit_chat_dialog, chat_name: chat_name)
        if add_chat_dialog {
...
//                    ScrollView(showsIndicators: false){
                    ...
                    case 4:
                        RagSettingsView(ragDir: "documents/" + (chat_name == "" ? "tmp_chat" : chat_name),
                                        chunkSize: $chunkSize,
                                        ...
                                        ragTop: $ragTop)
                    case 5:
                        DocsView(docsDir: "documents/" + (chat_name == "" ? "tmp_chat" : chat_name) + "/docs",
                                 ragDir: "documents/" + (chat_name == "" ? "tmp_chat" : chat_name),
                                 chunkSize: $chunkSize,
                                 chunkOverlap: $chunkOverlap,
                                 currentModel: $currentModel,
                                 comparisonAlgorithm: $comparisonAlgorithm,
