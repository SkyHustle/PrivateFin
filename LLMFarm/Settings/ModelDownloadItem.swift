...
struct ModelDownloadItem: View {
    ...
    @State var model_files: [[String: String]] = []
    ...
    init(modelInfo: DownloadModelInfo) {
        ...
        _modelName = State(initialValue: modelInfo.name ?? "Undefined")
        _model_files = State(initialValue: modelInfo.models ?? [])
        if model_files.count > 0 {
            _modelQuantization = State(initialValue: model_files[0]["Q"] ?? "")
            _download_url = State(initialValue: model_files[0]["url"] ?? "")
            _file_name = State(initialValue: model_files[0]["file_name"] ?? "")
            _status = State(initialValue: FileManager.default.fileExists(atPath: getFileURLFormPathStr(dir: "models", filename: file_name).path) ? "downloaded" : "download")
        }
    }
