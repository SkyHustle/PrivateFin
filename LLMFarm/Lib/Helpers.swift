...
func getCurrentModelFromStr(_ modelStr: String) -> EmbeddingModelType {
    switch modelStr {
    case "minilmMultiQA":
        EmbeddingModelType.minilmMultiQA
    case "distilbert":
        EmbeddingModelType.distilbert
    case "minilmAll":
        EmbeddingModelType.minilmAll
    default:
        EmbeddingModelType.minilmMultiQA
    }
}
...
func getComparisonAlgorithmFromStr(_ modelStr: String) -> SimilarityMetricType {
    ...
    case "euclidian":
        SimilarityMetricType.euclidian
    default:
        SimilarityMetricType.dotproduct
    }
}
...
public func getChatInfo(_ chat_fname: String) -> [String: AnyObject]? {
    do {
        let fileManager = FileManager.default
        let documentsPath = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first
...
public func duplicateChat(_ chat: [String: String]) -> Bool {
    if chat["chat"] != nil {
...
public func deleteChats(_ chats: [[String: String]]) -> Bool {
    do {
        let fileManager = FileManager.default
        let documentsPath = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first
        let destinationURL = documentsPath!.appendingPathComponent("chats")
...
public func removeFile(_ files: [[String: String]], dest: String = "models") -> Bool {
    do {
        let fileManager = FileManager.default
        let documentsPath = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first
...
public func get_model_info(_ model_path: String, full: Bool = false) -> [String: AnyObject] {
...
public func get_chats_list() -> [[String: String]]? {
    ...
    do {
        ...
        for chatfile_url in files {
            ...
            if chatfile.contains(".json") {
                ...
                var model = ""
                var message = ""
                var model_info: [String: AnyObject]
                var m_size: UInt64 = 0
...
public func getFileListByExts(dir: String = "models", exts: [String]) -> [[String: String]]? {
    var res: [[String: String]] = []
    do {
        let fileManager = FileManager.default
        let documentsPath = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first
...
func CreateChat(_ in_options: [String: Any], edit_chat_dialog: Bool = false, chat_name: String = "", save_as_template: Bool = false) -> Bool {
    do {
...
func get_downloadble_models(_ fname: String) -> [DownloadModelInfo]? {
    ...
    do {
        ...
        let jsonResult = try JSONSerialization.jsonObject(with: data, options: .mutableLeaves)
        let jsonResult_dict = jsonResult as? [[String: Any]]
        if jsonResult_dict == nil {
            return []
        }
        for row in jsonResult_dict! {
            let tmp_info = DownloadModelInfo(name: row["name"] as? String, models: row["models"] as? [[String: String]])
            res.append(tmp_info)
        }
        //        return jsonResult_dict
        return res
...
func load_chat_history(_ fname: String) -> [Message]? {
    ...
    do {
        ...
        let data = try Data(contentsOf: URL(fileURLWithPath: path), options: .mappedIfSafe)
        let jsonResult = try JSONSerialization.jsonObject(with: data, options: .mutableLeaves)
        let jsonResult_dict = jsonResult as? [[String: String]]
        if jsonResult_dict == nil {
            return []
        }
        for row in jsonResult_dict! {
            ...
            if row["id"] != nil {
                tmp_msg.id = UUID(uuidString: row["id"]!)!
            }
            if row["header"] != nil {
...
func GetRagDirRelPath(chat_name: String) -> String {
    "documents/" + (chat_name == "" ? "tmp_chat" : chat_name)
}
...
struct InputDoument: FileDocument {
    ...
    func fileWrapper(configuration _: WriteConfiguration) throws -> FileWrapper {
        FileWrapper(regularFileWithContents: input.data(using: .utf8)!)
    }
}
